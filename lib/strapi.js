...
export async function getActiveAnnouncement() {
  try {
    ...
    const data = await fetchStrapi(
      `/announcements?filters[active][$eq]=true&filters[$or][0][start_date][$null]=true&filters[$or][0][start_date][$lte]=${now}&filters[$or][1][end_date][$null]=true&filters[$or][1][end_date][$gte]=${now}&sort=createdAt:desc&pagination[limit]=1`
    )
    if (data?.data?.length > 0) {
