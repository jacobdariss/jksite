    ...
    <>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid var(--bd)', position: 'sticky', top: 64, zIndex: 50 }}>
        <div className="container" style={{ maxWidth: 860, display: 'flex', gap: 0, overflowX: 'auto' }}>
          {LEGAL_LINKS.map(l => (
            <Link key={l.href} href={l.href} className="legal-nav-link">{l.label}</Link>
          ))}
        </div>
      </div>
      ...
      <div className="container" style={{ maxWidth: 860, paddingBottom: 64 }}>
        <div style={{ background: '#FFF7EE', borderRadius: 16, padding: '28px 24px', borderLeft: '4px solid var(--o)' }}>
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', fontSize: '.88rem' }}>
            ...
            <span style={{ color: 'var(--bs)' }}>Mermoz VDN Dakar, Immeuble Zanardo, Sénégal</span>
          </div>
        </div>
      </div>

      <style>{`
        .legal-nav-link { padding: 12px 20px; font-size: .82rem; font-weight: 600; color: var(--bm); border-bottom: 2px solid transparent; white-space: nowrap; transition: color .2s; display: inline-block; }
        .legal-nav-link:hover { color: var(--o); }
