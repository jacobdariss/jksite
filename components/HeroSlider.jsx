...
export default function HeroSlider() {
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden' }}>
      {/* Track — toutes les slides côte à côte, glissement CSS */}
      <div style={{
        display: 'flex',
        ...
        {SLIDES.map((s, idx) => (
          <div key={idx} style={{ flex: '0 0 100%', background: s.bg, minWidth: 0 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: 48, alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <h1 style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(2rem,4.5vw,3rem)', lineHeight: 1.1, fontWeight: 700, letterSpacing: '-.02em', marginBottom: 14 }}>
                    {s.h1[0]}<em style={{ color: s.accent, fontStyle: 'italic' }}>{s.h1[1]}</em>{s.h1[2]}
                  </h1>
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 32 }}>
                    ...
                    <a href={s.cta2.href} className="btn btn-lg btn-ol" style={{ color: s.accent, borderColor: s.accent }}>{s.cta2.label}</a>
                  </div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: -16, left: -16, background: '#fff', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: 'var(--shl)' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: `linear-gradient(135deg,${s.accent},var(--ol))`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {['l', 'r'].map(d => (
        <button key={d} onClick={() => go(cur + (d === 'l' ? -1 : 1))}
          style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', zIndex: 10, [d === 'l' ? 'left' : 'right']: 16, background: 'rgba(255,255,255,.85)', border: 'none', width: 40, height: 40, borderRadius: '50%', fontSize: '1.2rem', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .2s' }}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: '16px 0', background: '#fff' }}>
        {SLIDES.map((_, i) => (
          <button key={i} onClick={() => go(i)} style={{
            ...
            background: i === cur ? 'var(--o)' : 'var(--bd)', cursor: 'pointer',
            transition: 'all .3s cubic-bezier(.4,0,.2,1)', padding: 0,
          }} />
        ))}
      </div>

      <style>{`@media(max-width:768px){.container [style*="1.1fr"]{grid-template-columns:1fr!important}.container [style*="1.1fr"]>div:last-child{display:none!important}}`}</style>
    </div>
  )
}
