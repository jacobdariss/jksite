...
const SLIDES = [
  {
    ...
    tagIcon: 'rack',
    h1: ['Une infrastructure ', 'souveraine', ' au cœur de l\'Afrique'],
    ...
    cta1: { label: 'Découvrir le datacenter →', href: '#dc', style: 'o' },
    cta2: { label: 'Voir nos offres', href: '#segments', style: 'ol' },
    ...
    bg: 'linear-gradient(160deg,#FFF7EE 0%,#FFE8D0 100%)',
  },
  {
    tag: 'Cloud Souverain · Paiements Africains',
    tagIcon: 'cloud',
    h1: ['Avant de parler de technologie, ', 'parlons de vous.', ''],
    ...
    cta1: { label: 'Découvrir nos solutions →', href: '#segments', style: 'o' },
    cta2: { label: 'Voir les offres', href: '#segments', style: 'ol' },
    ...
    bg: 'linear-gradient(160deg,#F5F3FF 0%,#EDE8F5 100%)',
  },
  {
    tag: 'Segment Institution',
    tagIcon: 'institution',
    h1: ['Vos obligations sont ', 'non négociables.', ' Notre engagement aussi.'],
    desc: 'Souveraineté totale des données, SLA avec pénalités, PRA documenté et testé, support 24/7 dédié.',
    cta1: { label: 'Offres Institution →', href: '/institution', style: 'p' },
    cta2: { label: 'Notre datacenter', href: '#dc', style: 'ol' },
    ...
    bg: 'linear-gradient(160deg,#F3F0FA 0%,#E8E0F5 100%)',
  },
]

const BTN_STYLES = {
  o: { background: 'var(--o)', color: '#fff' },
  ol: { background: 'transparent', color: 'var(--o)', border: '1.5px solid var(--o)' },
  p: { background: '#6B4C9A', color: '#fff' },
}
...
export default function HeroSlider() {
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden', background: s.bg, transition: 'background .5s' }}>
      {/* Arrows */}
      {['l','r'].map(d => (
        <button key={d} onClick={() => go(cur + (d==='l'?-1:1))}
          style={{ position:'absolute',top:'50%',transform:'translateY(-50%)',zIndex:10,
            [d==='l'?'left':'right']:16, background:'rgba(255,255,255,.8)',border:'none',
            width:40,height:40,borderRadius:'50%',fontSize:'1.2rem',cursor:'pointer',
            boxShadow:'0 2px 8px rgba(0,0,0,.1)',display:'flex',alignItems:'center',justifyContent:'center' }}>
          {d==='l'?'‹':'›'}
        </button>
      ))}
      ...
      <div className="container" style={{ padding: '56px 24px 48px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: 48, alignItems: 'center' }}>
          {/* Content */}
          <div>
            <div style={{ display:'inline-flex',alignItems:'center',gap:6,background:'rgba(232,93,4,.1)',border:'1px solid rgba(232,93,4,.2)',borderRadius:999,padding:'5px 14px',marginBottom:16,fontSize:'.72rem',fontWeight:700,color:'var(--o)' }}>
              {s.tag}
            </div>
            <h1 style={{ fontFamily:'var(--fd)',fontSize:'clamp(2rem,4.5vw,3rem)',lineHeight:1.1,fontWeight:700,letterSpacing:'-.02em',marginBottom:14 }}>
              {s.h1[0]}<em style={{ color:'var(--o)',fontStyle:'italic' }}>{s.h1[1]}</em>{s.h1[2]}
            </h1>
            <p style={{ fontSize:'.98rem',color:'var(--bs)',lineHeight:1.7,marginBottom:28,maxWidth:480 }}>{s.desc}</p>
            <div style={{ display:'flex',gap:12,flexWrap:'wrap',marginBottom:32 }}>
              <a href={s.cta1.href} className="btn btn-lg" style={BTN_STYLES[s.cta1.style]}>{s.cta1.label}</a>
              <a href={s.cta2.href} className="btn btn-lg" style={BTN_STYLES[s.cta2.style]}>{s.cta2.label}</a>
            </div>
      ...
      <div style={{ display:'flex',justifyContent:'center',gap:8,padding:'16px 0',background:'#fff' }}>
        {SLIDES.map((_,i) => (
          <button key={i} onClick={() => go(i)} style={{
            width: i===cur?24:8, height:8, borderRadius:999, border:'none',
            background: i===cur?'var(--o)':'var(--bd)', cursor:'pointer', transition:'all .3s',
          }} />
        ))}
      </div>

      <style>{`@media(max-width:768px){.hero-grid{grid-template-columns:1fr!important}.hero-grid>div:last-child{display:none}}`}</style>
    </div>
  )
}
